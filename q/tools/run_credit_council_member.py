#!/usr/bin/env python3
"""
Credit lead-lag council member.

Converts the signed market-level credit lead-lag signal into a T x N matrix so it
can participate as a directional council voter.

Writes:
  - runs_plus/council_credit_leadlag.csv
  - runs_plus/council_credit_leadlag_info.json
"""
...
def build_credit_council_member(
    signal: np.ndarray,
...
def main() -> int:
    t, n = _target_shape()
    sig_raw = _load_series(RUNS / "credit_leadlag_signal.csv")
    ...
    if t <= 0 or n <= 0:
        print("(!) Missing target shape for council credit member; skipping.")
        return 0
    ...
    direct = _env_syms(
        "Q_CREDIT_MEMBER_DIRECT_SYMBOLS",
        {
            "SPY",
            "QQQ",
            "DIA",
    ...
    inverse = _env_syms("Q_CREDIT_MEMBER_INVERSE_SYMBOLS", {"HYG", "LQD", "JNK"})
    attenuated = _env_syms(
        "Q_CREDIT_MEMBER_ATTENUATED_SYMBOLS",
        {"TLT", "IEF", "SHY", "BIL", "TIP", "GLD", "SLV", "UUP"},
    )
    attenuation = float(np.clip(float(os.getenv("Q_CREDIT_MEMBER_ATTENUATION", "0.30")), -1.0, 1.0))
    default_gain = float(np.clip(float(os.getenv("Q_CREDIT_MEMBER_DEFAULT_GAIN", "1.0")), -1.0, 1.0))

    mat, gains = build_credit_council_member(
        signal,
        names,
    ...
    np.savetxt(RUNS / "council_credit_leadlag.csv", np.asarray(mat, float), delimiter=",")
    ...
    (RUNS / "council_credit_leadlag_info.json").write_text(json.dumps(info, indent=2), encoding="utf-8")

    _append_card(
        "Council Member: Credit Lead-Lag ✔",
        (
    ...
    print(f"✅ Wrote {RUNS/'council_credit_leadlag.csv'}")
    print(f"✅ Wrote {RUNS/'council_credit_leadlag_info.json'}")
    return 0
